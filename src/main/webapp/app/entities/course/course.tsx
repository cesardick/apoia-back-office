  ...
  return (
    <div>
      <h2 id="course-heading" data-cy="CourseHeading">
        <Translate contentKey="apoiaBackOfficeApp.course.home.title">Courses</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="apoiaBackOfficeApp.course.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/course/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="apoiaBackOfficeApp.course.home.createLabel">Create new Course</Translate>
          </Link>
        </div>
      </h2>
        ...
        {courseList && courseList.length > 0 ? (
          <Table responsive>
            <thead>
              <tr>
                ...
                <th className="hand" onClick={sort('seoPublished')}>
                  <Translate contentKey="apoiaBackOfficeApp.course.seoPublished">Seo Published</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('seoPublished')} />
                </th>
                <th>
                  <Translate contentKey="apoiaBackOfficeApp.course.area">Area</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {courseList.map((course, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{course.seoPublished ? 'true' : 'false'}</td>
                  <td>{course.area ? <Link to={`/area/${course.area.id}`}>{course.area.id}</Link> : ''}</td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="apoiaBackOfficeApp.course.home.notFound">No Courses found</Translate>
            </div>
          )
        )}
      </div>
